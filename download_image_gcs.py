...
class ImageDownloader:
    ...
    def download_image(self, items):
        ...
        for item in items:
            ...
            try:
                ...
                # filenameは夢展望特有の処理が含まれているので適宜変更する
                filename = self.get_dir_path() + 'YT-' + item_id.split('/')[6]
                ...
                # 一度サーバーにおいてからクラウドストレージにコピーする。（ディレクトリ構造を保持）
                os.system('cp ' +  filename + ' /home/sensy/gcs_pub/item_images/yumetenbo/raw/'+ filename.split('/')[5])
                success.append({"id": item_id, "path": filename})
            except Exception as error:
                log(error)
                dl_err.append(str(item['id']))
                continue
            #os.system('cp ' +  filename + ' /home/sensy/s3/static/img/sensy/fashion/')
            time.sleep(interval)
    ...
    def get_dir_path(self):
        ...
        os.makedirs(last)
        # クラウドストレージ用のディレクトリも切っておく
        last_s3 = '/home/sensy/gcs_pub/item_images/yumetenbo/raw/' + str(last_int)
        os.makedirs(last_s3)
        return last + "/"
